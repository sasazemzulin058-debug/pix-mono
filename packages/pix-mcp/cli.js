...
function printHelp(log = console.log) {
  ...
  log("  pi install npm:@xynogen/pix-mcp\n");
  log("Then optionally run:");
  log("  pix-mcp init       Detect host configs and scaffold Pi imports");
  log("  pix-mcp init --dry-run");
}
...
export async function main(argv = process.argv.slice(2), log = console.log, error = console.error) {
  ...
  if (command === "install") {
    ...
    error("Use `pi install npm:@xynogen/pix-mcp` instead, then optionally run `pix-mcp init`.");
    return 1;
  }
